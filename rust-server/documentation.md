...
│   └── queries.rs        # Shared user/token lookup queries
│
├── features/
│   ├── mod.rs            # Feature route composition
│   ├── auth/             # Authentication feature
│   │   ├── mod.rs
│   │   ├── types.rs      # Auth request/response types and validation models
│   │   ├── queries.rs    # Auth-specific database operations
│   │   ├── service.rs    # Auth business logic (user/session/token flows)
│   │   ├── routes.rs     # Auth router for /api/auth/*
│   │   ├── tokens/       # JWT, cookie, and refresh-token helpers
│   │   └── handlers/     # Auth handler modules (login, logout, me, register, verify)
...
├── middleware/
│   ├── mod.rs
│   ├── auth.rs           # Cookie-based auth extractor and role checks
...
tests/
├── api.rs                # integration test entrypoint
├── api/
...
| `db/`       | Database connection and shared query helpers                                      |
| `features/`  | Auth and health feature modules with route composition                            |
| `middleware/` | CORS, logging, and cookie-based auth plumbing                                    |
| `utils/`     | API response, email, password, tracing, and validation helpers                    |
...
| File        | Purpose                                                                    |
| ----------- | -------------------------------------------------------------------------- |
| `auth.rs`   | Cookie-based `AuthUser` extractor and role checks                          |
| `logger.rs` | Route logger that prints method, path, status, and duration to stderr     |
| `cors.rs`   | Builds CORS layer from `FRONTEND_URL` with `Result` (no `expect`)         |

<details>
<summary><strong>auth.rs</strong></summary>

`AuthUser` reads the `access_token` cookie, verifies the JWT, and exposes the authenticated user ID and role to handlers like `GET /api/auth/me`.

`require_role()` is a small helper for role-gated handlers.
...
## 🏗️ Feature Types and Queries

### Authentication Types (`src/features/auth/types.rs`)

| Type                 | Purpose |
| -------------------- | ------- |
| `RegisterData`       | User registration input with validation |
| `AvailabilityQuery`  | Availability check query parameters |
| `ResendTokenData`    | Body for verification token resend |
| `VerifyQuery`        | Verification token query parameters |
| `LoginData`          | Login payload (`login`, `password`) |
| `MeResponse`         | Authenticated user response for `/me` |
| `Token`              | Simple token wrapper used in the auth flow |
| `LoginResponse`      | Login response body with user ID and role |
| `LoginSessionResult` | Internal service result for successful login |

**RegisterData validation:**

- `email` is normalized to lowercase and validated as email format.
- `username` is normalized to lowercase and validated for length and allowed characters.
- `password` must be 8-50 characters and include uppercase, lowercase, number, and special character.

**Login and verification flow:**

- `AvailabilityQuery` accepts either `email` or `username`, not both.
- `VerifyQuery` carries the verification token in the query string.
- `ResendTokenData` carries the verification token in the request body.

### Authentication Queries (`src/features/auth/queries.rs`)

Authentication-specific database operations now cover both verification and session management.

**Key functions:**

- `create_user()` - Inserts a user and returns the new UUID.
- `create_verification_token()` - Inserts a 24-hour verification token.
- `delete_user()` - Removes a user during compensating rollback.
- `verify_user()` - Marks an account as verified.
- `update_verification_token()` - Rotates a verification token during resend.
- `create_auth_session()` - Stores a hashed refresh token session row.
- `find_auth_session_by_hash()` - Looks up a session by hashed refresh token.
- `revoke_auth_session()` - Marks a session as revoked.

### Shared Database Queries (`src/db/queries.rs`)

Low-level shared lookups used by auth and verification flows.

**Key functions:**

- `find_user_by_email()` - Loads a full `User` by email.
- `find_user_by_username()` - Loads a full `User` by username.
- `find_user_by_id()` - Loads the identity fields needed by `/me`.
- `find_token_by_value()` - Loads verification token rows joined with user state.

### Authentication Service and Tokens (`src/features/auth/service.rs`, `src/features/auth/tokens/*`)

The auth service layer ties together validation, hashing, JWT signing, refresh-session storage, and cookie creation.

**Key functions:**

- `new_user()` - Wraps user creation and verification-token creation in a transaction.
- `find_login_user()` - Resolves login input as either email or username.
- `login_and_create_session()` - Verifies the password, checks account state, and signs the access token.
- `new_refresh_token()` - Generates a fresh refresh token string.
- `hash_refresh_token()` - Hashes the refresh token before storing it in the database.
- `sign_access_token()` - Signs access JWT claims for the authenticated user.
- `verify_access_token()` - Validates access JWT claims from the auth cookie.
- `access_cookie()` / `refresh_cookie()` - Build the HttpOnly cookies used by auth.
- `clear_access_cookie()` / `clear_refresh_cookie()` - Clear cookies during logout or session rotation.
...
2. Merge feature routes from `features::routes()`
...
`POST /api/auth/login` sets the `access_token` cookie. `GET /api/auth/me` reads that cookie through `AuthUser`. `POST /api/auth/logout` clears it.
...
- `APP_ENV` controls development, production, or test behavior.
- `PORT` sets the HTTP bind port.
- `DB_URL` and `DB_TEST_URL` select the active database URL.
- `FRONTEND_URL` is used by CORS and verification-link generation.
- `RESEND_API_KEY` and `TARPIT_DOMAIN` configure outbound email.
- `JWT_ACCESS_SECRET`, `JWT_ACCESS_TTL_SECONDS`, `REFRESH_TTL_SECONDS`, and `REFRESH_TOKEN_PEPPER` drive auth token handling.

Check `.env_example` for the complete variable list.
...
- Current API test modules: `tests/api/health.rs` and `tests/api/auth/{login,register,verify,support}.rs`.
- Additional unit-style coverage lives in `tests/unit.rs`, `tests/errors/app_error.rs`, `tests/features/auth/types.rs`, and `tests/utils/{password,validators}.rs`.
